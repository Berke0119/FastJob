...
class _SignUpState extends State<SignUp> with TickerProviderStateMixin {
  ...
  Future<Map<String, dynamic>?> analyzeProfileImage(String imageUrl) async {
    try {
      final response = await http.post(
        Uri.parse('http://192.168.1.46:3000/analyze-profile-photo'),
      ...
      print("HTTP error (image analysis): $e");
      return null;
    }
  }
  ...
  void _submitForm() async {
    ...
    if (isValid) {
      ...
      try {
        ...
        // 🔥 Burada hemen analiz yapılıyor
        final trustResult = await analyzeProfileImage(imageUrl!);
        int trustScore = trustResult?['score'] ?? 50;
        String trustReason = trustResult?['reason'] ?? 'Not available';
